...
export default function ExpertChat() {
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', text: t('expert_greeting') }
  ]);
  const listRef = useRef(null);

  useEffect(() => {
    setMessages([{ role: 'assistant', text: t('expert_greeting') }]);
  }, [lang, t]);
  ...
  async function ask(question) {
    ...
    return data?.answer || t('expert_error_ask');
  }
  ...
  async function onSend(e) {
    ...
    try {
      ...
      setMessages((m) => [...m, { role: 'assistant', text: t('expert_error') }]);
    } finally {
      setBusy(false);
    }
  }
      ...
      {!open ? (
        ...
        <div className="w-80 sm:w-96 h-96 bg-white rounded-2xl border border-gray-200 shadow-xl flex flex-col overflow-hidden">
          <div className="bg-emerald-600 text-white px-4 py-3 flex items-center justify-between">
            <div className="font-semibold">{t('expert_name')}</div>
          ...
          <form onSubmit={onSend} className="p-3 border-t bg-white flex items-center gap-2">
            <input
              ...
              placeholder={t('expert_placeholder')}
              value={input}
              onChange={(e) => setInput(e.target.value)}
            />
            <button type="submit" disabled={busy} className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white rounded-lg px-3 py-2 text-sm">{t('expert_send')}</button>
          </form>
        </div>
      )}
    </div>
