...
export default function LatestSchemesPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setError('');
      try {
        const res = await api.get('/schemes', { params: { lang } });
  ...
  return (
    <div className="space-y-8">
      ...
      {items.length === 0 && !loading && (
        <div className="text-center text-gray-500 py-8">{t('schemes_no_results') || 'No schemes available'}</div>
      )}
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {items.map((it, idx) => (
          <Card
            key={idx}
            title={it.short_name || it.name || 'Scheme'}
            footer={it.ministry || ''}
            badge={it.short_name}
            href={it.more_info || it.link || undefined}
          >
